...
async def magaza(event):
    ...
    await event.edit('**[⚝ 𝑭𝑨𝑺𝑻 𝑼𝑺𝑬𝑹𝑩𝑶𝑻 ⚝](@TheFastSupp) Plugin Mağazası**\n__Versiya 1.0__\n\n`🔎 Plugin\'i axtarıram... Xaiş biraz gözlə.`')
    split = plugin.split()
    if plugin == '':
        plugin = 'Son yüklənən'
    ...
    result = f'**[⚝ 𝑭𝑨𝑺𝑻 𝑼𝑺𝑬𝑹𝑩𝑶𝑻 ⚝](@TheFastSupp) Plugin Mağazası**\n\n**🔎 Axtarış:** `{plugin}`\n**🔢 Nəticələr: __({len(plugins)})__**\n➖➖➖➖➖\n\n'
...
async def sinstall(event):
    ...
    try:
        ...
        return await event.edit(f'**[⚝ 𝑭𝑨𝑺𝑻 𝑼𝑺𝑬𝑹𝑩𝑶𝑻 ⚝](@TheFastSupp) Plugin Mağazası**\n\n**⚠️ Xəta:** `Plugin xətalıdır. {e}`\n**XAİŞ BUNU İDARƏÇİLƏRƏ BİLDİRİN!**')
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                # FASTPY
                Fastpy = re.search('\"\"\"FASTPY(.*)\"\"\"', dosy, re.DOTALL)
                if not Fastpy == None:
