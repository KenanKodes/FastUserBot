...
async def show_current_gdrove_folder(event):
    if parent_id:
        ...
        await event.edit(
            f"FastUserBot'um faylları [buraya]({folder_link}) uploadlıyır.")
    else:
        await event.edit(
            f"FastUserBot'um faylları Google Drive'ın kök qovluğuna uploadlıyır.\
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "FastUserBot işlədilərək yükləndi.",
        "mimeType": mime_type,
    }
    if parent_id:
